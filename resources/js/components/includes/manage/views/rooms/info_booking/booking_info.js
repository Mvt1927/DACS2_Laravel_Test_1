...
class Booking_info extends Component {
    constructor(props) {
        super(props);
        ...
        if (data != null) {
            const filteredData = data.filter(element => {
                return element.id.toString().toLowerCase().includes(searchString.toLowerCase())
                    ...
                    || element.date.toString().toLowerCase().includes(searchString.toLowerCase())
    ...
    save = (event, id) => {
        ...
        if (this.state[id] != null && this.state[id] == false) {
            ...
            let url = MyGlobleSetting.url + '/api/control/post/edit-bookroom-reserve-stats';
            axios.post(url, bookroom_reserve_stats)
                .then(response => {
                    this.getBookroom_reserve();
                    this.getBookroom_reserveData();
                    this.setState({
                ...
                return (
                    <CTableRow id={i.id} key={i.id}>
                        ...
                        <CTableDataCell style={{ padding: "1.5px" }}>
                            <CFormSelect defaultValue={i.stats} onChange={event => this.isChange(event, i.id, i.stats)}>
                    ...
                    <CContainer>
                        <CRow>
                            <CCol xs={12}>
                                <CRow>
                                    <CCol xs={8}>
                                        <CFormLabel htmlFor="Find_Reservation_FormControlInput">Reservation code or name</CFormLabel>
                                        <CInputGroup>
                                            <CFormInput type="text" id="Find_Reservation_FormControlInput" name="Find_Reservation_FormControlInput" placeholder="Reservation code or name" defaultValue={this.state.Find_Reservation_FormControlInput} onChange={(event) => this.find(event)} />
                            ...
                            <CCol xs={12}>
                                <CTable hover bordered className='cursor-pointer'>
                                    <CTableHead className="thead-light">
                                        <CTableRow >
                                            <CTableHeaderCell scope='col' className="col-2 text-center">Reservation code</CTableHeaderCell>
                                            ...
                                            <CTableHeaderCell scope='col' className="col-2 text-center">Date</CTableHeaderCell>
                                            <CTableHeaderCell scope='col' className="col-1 text-center">Stats</CTableHeaderCell>
